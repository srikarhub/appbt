...
class Bt_apirequest {
   ...
   function __construct($guid = ""){
   		$tmp = "id, guid, appGuid, clientApiKey, clientRemoteAddress, clientUserAgent, ";
		$tmp .= "requestDirectory, requestCommand, requestStatus, errorMessage, errorCode,";
		$tmp .= "requestMethod, dateStampUTC, appUserGuid, deviceId, deviceModel,";
		$tmp .= "deviceLatitude, deviceLongitude ";
	...
	function fnDelete($guid = ""){
		if($guid != ""){
			$strSql = "DELETE FROM " . TBL_API_REQUESTS . " WHERE guid = '". $guid . "'";
	...
	function fnInsert(){
		if($this->infoArray['guid'] != ""){
			$strSql = "INSERT INTO " . TBL_API_REQUESTS . " (";
			//loop field names
	...
	function fnUpdate(){
		if($this->infoArray['guid'] != ""){
			$strSql = "UPDATE " . TBL_API_REQUESTS . " SET ";
			//loop field names
			foreach ($this->infoArray as $key=>$value){
				if(strtoupper($key) != "ID"){
