<?php  require_once("../../config.php");
	...
	//fnIsValidAPNSDeviceToken...
	function fnIsValidAPNSDeviceToken($deviceToken){
	...
	//fnProcessAPNSFeedback...
	function fnProcessAPNSFeedback($appGuid, $certPath, $certPassword, $useDev = true){
		
		//return...
		$bolPassed = true;
		$apnURL = "";
		
		//make sure certificate exists at certPath...
		if(is_file($certPath)){
			if($useDev){
				$apnURL = "ssl://feedback.sandbox.push.apple.com:2196";
			}else{
				$apnURL = "ssl://feedback.push.apple.com:2196";
			}
		}else{
			$bolPassed = false;
		}
		...
		if($bolPassed){
			...
			/* 	Ports are used by APNS
				----------------------------------
				1. Outgoing port 5223 (APN) 
				2. Outgoing port 2195 (PUSH) 
				3. Outgoing port 2196 (FEEDBACK) 
				4. Outgoing Port 443 (SSL) 
			*/
			...
			@stream_context_set_option($stream_context, 'ssl', 'local_cert', $certPath);
			@stream_context_set_option($stream_context, "ssl", "passphrase", $certPassword);
			$fp = @stream_socket_client($apnURL, $errCode, $errString, 2, STREAM_CLIENT_CONNECT, $stream_context);
			...
			if(!$fp){
				...
				//get data from service, close conn...
			...
			if($data === false){
				...
				//read the data...
				$itemCount = strlen($data) / 38;
	
				//loop through each item...
				for($i = 0; $i < $itemCount; $i++){
					$offset = $i * 38;
	
					//timestamp tells us when token was removed...	
					$timestamp = substr($data, $offset, 4);
					$timestamp = hexdec(bin2hex($timestamp));
	
					//length of the device token...
					$length = substr($data, $offset + 4, 2);
					$length = hexdec(bin2hex($length));
	
					//remaining 32 bytes contain the device token...
					$token = substr($data, $offset + 6, 32);
					$token = bin2hex($token);
	
					//remove device from registered list...
					if(strlen($token) > 1 && $appGuid != ""){
						
						//remove device from list...
						$strSql = "DELETE FROM " . TBL_APN_DEVICES . " WHERE appGuid = '" . $appGuid . "' AND deviceToken = '" . $token . "' ";
						if($useDev){
				...
				$tmpDeviceCount = fnGetOneValue("SELECT Count(*) FROM " . TBL_APN_DEVICES . " WHERE appGuid = '" . $appGuid . "'", APP_DB_HOST, APP_DB_NAME, APP_DB_USER, APP_DB_PASS);
				$tmp = "UPDATE " . TBL_APPLICATIONS . " SET deviceCount = '" . $tmpDeviceCount . "'";
				$tmp .= " WHERE guid = '" . $appGuid . "'";
	...
	//fnConnectToAPNServer
	function fnConnectToAPNS($appGuid, $certPath, $certPassword, $useDev = true){

		//url depends on "useDev" or not...
		$apnURL = "";
		...
		*/
		...
		@stream_context_set_option($stream_context, "ssl", "local_cert", $certPath);
		@stream_context_set_option($stream_context, "ssl", "passphrase", $certPassword);
		$fp = @stream_socket_client($apnURL, $err, $errStr, 60, STREAM_CLIENT_CONNECT | STREAM_CLIENT_PERSISTENT, $stream_context);
	...
	//fnDisconnectFromAPNS...
	function fnDisconnectFromAPNS($fp){
		if(is_resource($fp)){
			return fclose($fp);
		}
		return false;
