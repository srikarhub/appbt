<?php   require_once("../../config.php");
	...
	//get possible contextMenuNickname...
	$contextMenuNickname = fnGetReqVal("json_contextMenuNickname", "", $myRequestVars);
	$contextMenuItemId = fnGetReqVal("json_contextMenuItemId", "", $myRequestVars);
	...
	if($isFormPost){
		...
		if($bolPassed){
			...
			if($tmpId != ""){
				$bolPassed = false;
				$strMessage .= "<br>This nickname is already in use, no duplicates allowed.";
			}
		}
		...
		if($bolPassed && $contextMenuNickname != ""){
			...
			if($tmpId == ""){
				$bolPassed = false;
				$strMessage .= "<br>No \"menu\" for the nickname for the context menu?";
			}else{
		...
		if($bolPassed){
			...
			*/
			...
			echo "<div style='padding-top:5px;padding-bottom:5px;color:#000000;font-family:monospace;'>";
					$displayJson = "{\"itemId\":\"" . $BT_itemId . "\", \"itemType\":\"" . $itemType . "\", \"itemNickname\":\"" . fnFormOutput($nickname) . "\", ";
