<?php   require_once("../../config.php");
	...
	$formEl = fnGetReqVal("formEl", "", $myRequestVars);
	$fileNameOrURL = fnGetReqVal("fileNameOrURL", "", $myRequestVars);
	...
	if(strtoupper($command) == "UPLOADFILE"  && $bolPassed){
		...
		if($bolPassed){
			...
			if($extCount > 0){
				$bolPassed = false;
				$strMessage .= "<br/><b>Duplicate File</b>. This file manager already has a file named <b>" . $userfile_name . "</b>, duplicates are not allowed.";
			}
		}
	...
	$scriptName = "bt_pickerFile.php";
	$recsPerPage = 10;
	//if($searchFolder == "/images") $recsPerPage = 28;

	$totalPages = 1;
	$currentPage = 1;
	$firstRec = 0;
	$lastRec = 0;
	$defaultSort = "F.modifiedUTC";
	$defaultUpDown = "DESC";
	$colCount = 7;
	...
	$qVars .= "&screenGuid=" . $screenGuid . "&sortColumn=" . $sortColumn . "&sortUpDown=" . $sortUpDown;
	$qVars .= "&currentPage=" . $currentPage . "&searchFolder=" . $searchFolder . "&formEl=" . $formEl;
	$qVars .= "&formEl=" . $formEl . "&fileNameOrURL=" . $fileNameOrURL;
...
<script type='text/javascript'>
	function selectFile(theVal){
		try{
			parent.document.forms[0].<?php echo $formEl;?>.value = theVal;
...
<input type="hidden" name="formEl" id="formEl" value="<?php echo $formEl;?>">
<input type="hidden" name="fileNameOrURL" id="fileNameOrURL" value="<?php echo $fileNameOrURL;?>">
    ...
    <fieldset class='colorLightBg minHeightShadowbox' style='-moz-border-radius:0px;border-radius:0px;'>
       	...
       	<div class='contentBox colorLightBg' style='-moz-border-radius:0px;border-radius:0px;border-bottom:0px;'>
            ...
            <div class="contentBand colorBandBg" style='-moz-border-radius:0px;border-radius:0px;'>
                ...
                <span sytle='font-weight:normal;'>(this window will close when you select a file)</span>
            </div>
            <div style='padding:10px;'>
                    
                    <table style='width:100%;'>
                    	<tr>
    						<td style='width:150px;vertical-align:top;padding-right:10px;padding-top:3px;border-right:1px solid #999999;'>
                                ...
                                <table cellspacing='0' cellpadding='0' width="100%">
                                    ...
                                    <tr>
                                        <td nowrap style='padding:5px;padding-right:10px;'>
                                   			<a title="Show" href="<?php echo $scriptName . "?formEl=". $formEl . "&fileNameOrURL=" . $fileNameOrURL . "&appGuid=" . $appGuid . "&searchFolder=";?>"><?php echo fnSelectedFolder("", $searchFolder);?>All Files (<?php echo fnCountFileInFolder("", $appGuid);?>)</a> 
										</td>
                                    </tr>
                                    <tr>
                                        <td nowrap style='padding:5px;padding-right:10px;'>
                                   			<a title="Show" href="<?php echo $scriptName . "?formEl=". $formEl . "&fileNameOrURL=" . $fileNameOrURL . "&appGuid=" . $appGuid . "&searchFolder=/documents";?>"><?php echo fnSelectedFolder("/documents", $searchFolder);?>Documents (<?php echo fnCountFileInFolder("/documents", $appGuid);?>)</a> 
										</td>
                                    </tr>
                                    <tr>
                                        <td nowrap style='padding:5px;padding-right:10px;'>
                                   			<a title="Show" href="<?php echo $scriptName . "?formEl=". $formEl . "&fileNameOrURL=" . $fileNameOrURL . "&appGuid=" . $appGuid . "&searchFolder=/images";?>"><?php echo fnSelectedFolder("/images", $searchFolder);?>Images (<?php echo fnCountFileInFolder("/images", $appGuid);?>)</a> 
										</td>
                                    </tr>
                                    <tr>
                                        <td nowrap style='padding:5px;padding-right:10px;'>
                                   			<a title="Show" href="<?php echo $scriptName . "?formEl=". $formEl . "&fileNameOrURL=" . $fileNameOrURL . "&appGuid=" . $appGuid . "&searchFolder=/audio";?>"><?php echo fnSelectedFolder("/audio", $searchFolder);?>Audio (<?php echo fnCountFileInFolder("/audio", $appGuid);?>)</a> 
										</td>
                                    </tr>
                                    <tr>
                                        <td nowrap style='padding:5px;padding-right:10px;'>
                                   			<a title="Show" href="<?php echo $scriptName . "?formEl=". $formEl . "&fileNameOrURL=" . $fileNameOrURL . "&appGuid=" . $appGuid . "&searchFolder=/video";?>"><?php echo fnSelectedFolder("/video", $searchFolder);?>Video (<?php echo fnCountFileInFolder("/video", $appGuid);?>)</a> 
										</td>
                                    </tr>
                                    <tr>
                                        <td nowrap style='padding:5px;padding-right:10px;'>
                                   			<a title="Show" href="<?php echo $scriptName . "?formEl=". $formEl . "&fileNameOrURL=" . $fileNameOrURL . "&appGuid=" . $appGuid . "&searchFolder=/phpscripts";?>"><?php echo fnSelectedFolder("/phpscripts", $searchFolder);?>PHP Scripts (<?php echo fnCountFileInFolder("/phpscripts", $appGuid);?>)</a> 
										</td>
                                    </tr>
                                ...
                                <table cellspacing='0' cellpadding='0' width="100%" style='margin-top:5px;'>
                            		...
                            		<?php if($strMessage == "") { ?>
                                        ...
                                        <tr>
                                            ...
                                            <td colspan='2' nowrap class="tdSort">
                                        ...
                                        if($totalRecs > 0){
                                            ...
                                            if($res){
			                                    ...
			                                    //show image size if we have it...
													...
													//inserting file name or URL?
													$pickVal = $fileName;
													$pickURL = $appDataURL . "/" . ltrim(strtolower($filePath), "/") . "/" . ltrim($fileName, "/");
													$pickPath = $appDataDirectory . "/" . ltrim(strtolower($filePath), "/") . "/" . ltrim($fileName, "/");
													if(strtoupper($fileNameOrURL) == "URL"){
														$pickVal = $pickURL;
													}
													$pickLink = "<a href='#' title=\"" . $fileName . "\" onClick=\"selectFile('" . $pickVal . "');return false\">select</a>";
													...
													//open / unzip URL's..
													$openLink = "<a href='" . $appDataURL . "/" . strtolower($filePath) . "/" . $fileName . "' target='_blank'>open</a>";
													$unzipLink = "";
                                                    ...
                                                    echo "\n\n<tr id='i_" . $BT_fileId . "' class='" . $css . "'>";
                                                        echo "\n<td class='data' style='padding-left:0px;vertical-align:middle;'>";
                                                            echo fnFileTypeIcon($fileName) . " " . $label;
                                                        echo "</td>";
                                                        echo "\n<td class='data' style='vertical-align:middle;'>";
                                                            echo $unzipLink;
														echo "</td>";
                                                        echo "\n<td class='data' style='vertical-align:middle;'>";
                                                            echo $pickLink . " | " . $openLink;
														echo "</td>";
